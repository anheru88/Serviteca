...
public class Admin_BD {
	...
	public Cursor BuscarServicio(long id) {
		Leer();
		Cursor c = bd.rawQuery(" SELECT * FROM Servicios WHERE _id =? ",
				new String[] { id + "" });
		c.moveToFirst();
		return c;
	}
	...
	public Cursor AutoComplete(String textSearch) {
		Cursor c = bd.rawQuery("SELECT rowid  AS _id, placa AS item  FROM  Vehiculos WHERE placa LIKE '" + textSearch + "%' ",null);
		c.moveToFirst();
		return c;
	}
